...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    [Header("Hand Config")]
    private static bool isRightHandNext = true;
    private static float lastShootTime = 0f;

    public CrosshairManager crosshairManager;

    [Tooltip("Which hand this instance of the script refers to")]
    ...
    void Update()
    {
        ...
        if (activeWeapon != null && m_WeaponSwitchState == WeaponSwitchState.Up)
        {
            ...
            bool fireInput = m_InputHandler.GetFireInputDown();

            if (fireInput && Time.time - lastShootTime >= activeWeapon.DelayBetweenShots)
            {
                if ((isRightHand && isRightHandNext) || (!isRightHand && !isRightHandNext))
                {
                    bool hasFired = activeWeapon.HandleShootInputs(true, false, false);
                    ...
                    if (hasFired)
                    {
                        ...
                        // Update the shared cooldown and toggle the hand
