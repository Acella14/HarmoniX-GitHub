...
public class WeaponController : MonoBehaviour
{
    ...
    // Removed CrosshairData properties
    ...
    [Tooltip("Rotation angle for the recoil")]
    public Vector3 RecoilRotation = Vector3.zero;

    [Tooltip("Duration for the recoil effect in seconds")]
    ...
    [Tooltip("Prefab of muzzle words")]
    public GameObject MuzzleWordsPrefab;
    ...
    private Vector3 muzzleWordsOffset = new Vector3(0f, 0.2f, 0f);
    ...
    void UpdateRecoil()
    {
        if (isRecoiling)
        {
            float elapsedTime = Time.unscaledTime - recoilStartTime;
            float kickUpDuration = RecoilDuration / 4;
            float fallBackDuration = RecoilDuration * 3 / 4;

            if (elapsedTime < kickUpDuration)
            {
                float t = elapsedTime / kickUpDuration;
                transform.localRotation = Quaternion.Slerp(originalRotation, targetRotation, t);
            }
            else if (elapsedTime < RecoilDuration)
            {
                float t = (elapsedTime - kickUpDuration) / fallBackDuration;
    ...
    void HandleShoot()
    {
        ...
        if (WeaponAnimator)
        {
            Debug.Log("Setting Shoot trigger"); // Add this line for debugging
            WeaponAnimator.SetTrigger(k_AnimShootParameter);
        }

        if (RecoilRotation != Vector3.zero)
        {
            targetRotation = Quaternion.Euler(transform.localRotation.eulerAngles + RecoilRotation);
            originalRotation = transform.localRotation;
            recoilStartTime = Time.unscaledTime;
            isRecoiling = true;
        }
        ...
        if (MuzzleFlashPrefab != null)
        {
            ...
            GameObject muzzleWordInstance = Instantiate(MuzzleWordsPrefab, WeaponMuzzle.position + muzzleWordsOffset,
                WeaponMuzzle.rotation, WeaponMuzzle.transform);

            if (UnparentMuzzleFlash)
            {
                muzzleFlashInstance.transform.SetParent(null);
                muzzleWordInstance.transform.SetParent(null);
            }

            Destroy(muzzleFlashInstance, 2f);
            Destroy(muzzleWordInstance, 2f);
        }
